...
if(isset($_GET['msg'])&&$_GET['msg']!=''){	
	...
	case 'inf':
		$msg='City not added Successfully !!';
	...
	case 'upf':
		$msg='City not updated Successfully !!';
		$class='alert alert-danger alert-dismissible';
		$iconmsg='close-circle-o';
	break;
	case 'dls':
	...
	case 'dlf':
		$msg='City not deleted successfully !!!!';
		$class='alert alert-danger alert-dismissible';
		$iconmsg='close-circle-o';
	break;
...
$countyid=$_GET['id'];
$delQry=mysqli_query($conn,"delete from `city` where `id`='$did'");
	if($delQry){
		header("location:addcity.php?msg=dls&id=$countyid");
	}else{
		header("location:addcity.php?msg=dlf&id=$countyid");
	}
}
...
$sqlQry="INSERT INTO `city`(`name`,`imagepath`, `slug`, `status`,`cid`) VALUES ('$firstline','$imgName1','$slug','1','$countyidnew')";
$execQry=mysqli_query($conn,$sqlQry);
			

if($execQry){
	header("location:addcity.php?msg=ins&id=$countyid");
}else{
	header("location:addcity.php?msg=inf&id=$countyid");
}
...
$nwsimg1=$_FILES['image1']['name'];
	...
	if(!$nwsimg1==''){
		...
		$resImgQry1=mysqli_fetch_row(mysqli_query($conn,"select `imagepath` from `city` where `id` = '$id'"));
	    $oldimg1=$resImgQry1[0];
	    $imgName1=$oldimg1;	
	}

$eiid=base64_encode($id);
$sqlQry="UPDATE `city` SET `name`='$firstline',`slug`='$secondline',`imagepath`='$imgName1' WHERE `id`='$id' ";
$execQry=mysqli_query($conn,$sqlQry);
	
if($execQry){
	header("location:addcity.php?msg=ups&id=$countyid");
}else{
	header("location:addcity.php?msg=upf&id=$countyid");
}
...
<head>
    ...
    <title>Joseph Shadel || City</title>
        ...
        <div class="page-head">
          ...
          <ol class="breadcrumb page-head-nav">
            ...
            <li> <a href="county.php">Counties</a></li>
                <li class="active"> <a href="javascript:void(0;">City- For County '<?php echo getcountynamebyid($conn,$countyid)?>'</a></li> 
          </ol>
        </div>
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="panel panel-default panel-border-color panel-border-color-primary">
                <div class="panel-heading panel-heading-divider">Add Cities </div>
                ...
                <div role="alert" class="<?php echo $class?>">
                  ...
                  <?php }?>
                  <form  style="border-radius: 0px;" class="form-horizontal group-border-dashed" id="cityform" name="cityform" method="post" enctype="multipart/form-data" >
                    <?php 
					if(isset($_GET['eid'])&&$_GET['eid']!=''){ 
                        $eid=base64_decode($_GET['eid']);
                        $execQry=mysqli_fetch_row(mysqli_query($conn,"select * from `city` where `id`='$eid'"));
                 ...
                 <div class="col-sm-3">
                   ...
                   <img src="<?php echo $baseurl?>/photos/<?php echo $execQry[3]?>" style="width:86px;  height:70px"/>
                       </div> 
                    </div>
              ...
              <div class="panel panel-default panel-table">
                ...
                <div class="panel-body">
                  <table id="table1" class="table table-striped table-hover table-fw-widget">
                    ...
                    <tbody>
                        <?php 
  	$sqlQry=mysqli_query($conn,"select * from `city` where `cid`='$countyid'  order by `id` ASC ");
	$i=0;
  ...
  ?> 
                      <tr class="<?= $class?> ">
                        ...
                        <td class="center" ><a href="addcity.php?did=<?php echo base64_encode($fetch['id'])?>&id=<?php echo base64_encode($countyid)?>"  class="confirm-dialog" onClick="return confirm(' Are you sure you want to delete !!')"><img src="<?php echo $baseurl;?>/images/trash.png" style="width:20px; height:20px"></a></td>
	<td class="center" ><a href="addcity.php?eid=<?php echo base64_encode($fetch['id']) ?>&id=<?php echo base64_encode($countyid)?>"><img src="<?php echo $baseurl;?>/images/pen.png" style="width:20px; height:20px">	</a></td>
		<td class="center" >
       <div class="be-checkbox be-checkbox-color inline"> <input class='uniform ap' type="checkbox" id="check<?php echo $fetch['id']  ?>" value="<?php echo $fetch['status']  ?>" onClick="updateStatus('<?php echo $fetch['id'];  ?>','city',5)" <?php if($fetch['status']==1){echo 'checked';} ?>>
