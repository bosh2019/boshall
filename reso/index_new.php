...
echo"<th>Status</th>";
/*get loop property*/
$loopsql="SELECT * FROM `loop_property` WHERE `status`=1";
$results_loop=$conn->query($loopsql);
if($results_loop->num_rows>0)
{
	while ($row= $results_loop->fetch_assoc()) {

/*end loop property*/



$results = $rets->Search('RESI', ''.$row["shortname"].'', 'StandardStatus=ACT',['Limit' => 100000]);
...
$r["PostalCode"] = $conn->real_escape_string($r["PostalCode"]);
...
$address=$r["StreetName"]." ".$r["CountyOrParish"]." ".$r["StateOrProvince"];
	...
	$insertQ=1;
	    if($insertQ)
	    {
			...
			echo"<th>Inserted</th>";
			echo"<th>('RESI', '".$row["shortname"]."', 'StandardStatus=ACT',['Limit' => 1])</th>";
			echo"</tr>";
	    }
	    else
	    {
 ...
 }/*end while loop array*/
}/*end if loop array*/

$dropSql="DROP TABLE `mlspindata_master`";
$Qdrop=$conn->query($dropSql);
if($Qdrop)
{
	$renameSql="ALTER TABLE `temp_mlspindata_master` RENAME `mlspindata_master`";
	$Qrename=$conn->query($renameSql);
	if($Qrename)
	{
		$createSql="CREATE TABLE `temp_mlspindata_master` LIKE `mlspindata_master`";
